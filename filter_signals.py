from __future__ import annotations

import json
import re
from collections import Counter
from pathlib import Path


ROOT = Path(__file__).resolve().parent
RAW = ROOT / "data" / "raw_signals.jsonl"
OUT = ROOT / "data" / "filtered_signals.jsonl"

FILTER_VERSION = "rules-v2"


GENERIC_DROP_TITLES = {
    "haberler",
    "haber",
    "duyurular",
}


# Terms are normalized with the same Turkish-safe function as titles.
HIGH_SIGNAL_TERMS = {
    # Weather / safety
    "uyarı",
    "sağanak",
    "yağış",
    "fırtına",
    "kuvvetli rüzgâr",
    "kuvvetli rüzgar",
    "aşırı sıcak",
    "sıcaklık",
    "yangın",
    "kapatıldı",
    "kapalı",

    # Mobility / infrastructure
    "ulaşım",
    "trafik",
    "yol",
    "cadde",
    "sokak",
    "köprü",
    "tünel",
    "istasyon",
    "metro",
    "tramvay",
    "izban",
    "otobüs",
    "vapur",
    "sefer",
    "altyapı",
    "yenileme",

    # Utility / disruption
    "elektrik kesintisi",
    "su kesintisi",
    "doğalgaz",
    "arıza",

    # Direct public usefulness
    "ücretsiz",
    "indirimli",
}


EVENT_TERMS = {
    "etkinlik",
    "festival",
    "konser",
    "kutlanacak",
    "coşkusu",
    "bayram",
    "sergi",
    "kayıt",
}


PR_TERMS = {
    "baş tacımız",
    "ziyaret etti",
    "teşekkür",
    "mesajı",
    "ağırladı",
    "buluştu",
    "protokol",
    "heyet",
    "gurur",
    "ödül",
    "başarı",
    "eğitimini tamamladı",
    "personeline eğitim",
    "personele özel eğitim",
    "yapay zekâ desteği",
    "yapay zeka desteği",
}


# Retrospective / ceremony-style headlines are usually poor "what is
# happening now?" map signals. Strong safety/transport signals still win.
RETROSPECTIVE_TERMS = {
    "buluşma noktası oldu",
    "yolculuk yaptı",
    "avrupa beşincisi",
    "çifte gurur",
}


def text(value) -> str:
    return str(value or "").strip()


def normalize(value) -> str:
    """
    Turkish-safe, deterministic normalization.

    Python lower()/casefold() alone does not behave like Turkish locale for
    I / İ / ı. For matching purposes we deliberately collapse all i variants.
    """
    value = text(value)

    translation = str.maketrans({
        "I": "i",
        "İ": "i",
        "ı": "i",
    })

    value = value.translate(translation).casefold()
    return " ".join(value.split())


def contains_term(haystack: str, term: str) -> bool:
    """
    Match complete words/phrases instead of arbitrary substrings.
    This prevents 'yol' from matching 'yolculuk'.
    """
    term_n = normalize(term)

    pattern = (
        r"(?<!\w)"
        + re.escape(term_n).replace(r"\ ", r"\s+")
        + r"(?!\w)"
    )

    return re.search(pattern, haystack, flags=re.UNICODE) is not None


def term_hits(title_n: str, terms: set[str]) -> list[str]:
    return sorted(
        term
        for term in terms
        if contains_term(title_n, term)
    )


def classify(row: dict) -> tuple[str, str]:
    source_id = text(row.get("source_id"))
    title = text(row.get("title"))
    title_n = normalize(title)
    raw_summary = normalize(row.get("raw_summary"))

    # Collector errors / technical probes never belong on the public map.
    if raw_summary.startswith("error:"):
        return "DROP", "collector_error"

    if source_id == "bursa_acik_yesil_catalog":
        return "DROP", "page_watch_not_an_event"

    # Old AFAD page-watch row from the earliest experiment.
    if source_id == "afad_event_service" and row.get("event_id") is None:
        return "DROP", "afad_non_event_record"

    # Public-map earthquake threshold.
    if source_id == "afad_event_service":
        try:
            magnitude = float(row.get("magnitude"))
        except (TypeError, ValueError):
            return "DROP", "afad_missing_magnitude"

        if magnitude >= 4.0:
            return "KEEP", "earthquake_m4_plus"

        if magnitude >= 3.0:
            return "KEEP", "earthquake_m3_plus"

        return "DROP", "earthquake_below_m3"

    if not title:
        return "DROP", "missing_title"

    if title_n in {normalize(x) for x in GENERIC_DROP_TITLES}:
        return "DROP", "generic_navigation_title"

    high_hits = term_hits(title_n, HIGH_SIGNAL_TERMS)
    event_hits = term_hits(title_n, EVENT_TERMS)
    pr_hits = term_hits(title_n, PR_TERMS)
    retrospective_hits = term_hits(title_n, RETROSPECTIVE_TERMS)

    # Safety / disruption / transport beats PR-style wording.
    if high_hits:
        return "KEEP", "high_signal:" + ",".join(high_hits[:3])

    # Clear retrospective headlines should not become live map signals just
    # because they happen to contain a word like "etkinlik".
    if retrospective_hits:
        return (
            "DROP",
            "retrospective:" + ",".join(retrospective_hits[:3])
        )

    if pr_hits:
        return "DROP", "likely_pr:" + ",".join(pr_hits[:3])

    if event_hits:
        return "KEEP", "public_event:" + ",".join(event_hits[:3])

    # AKOM is safety/emergency-focused. Unknown titles stay reviewable.
    if source_id == "akom_istanbul_news":
        return "MAYBE", "akom_needs_review"

    if source_id == "izmir_bb_news":
        return "MAYBE", "municipal_news_needs_review"

    return "MAYBE", "unclassified_source"


def load_rows():
    if not RAW.exists():
        raise SystemExit(f"Missing input file: {RAW}")

    rows = []
    bad_lines = 0

    with RAW.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            line = line.strip()

            if not line:
                continue

            try:
                rows.append(json.loads(line))
            except json.JSONDecodeError:
                bad_lines += 1
                print(f"SKIP invalid JSON line {line_no}")

    return rows, bad_lines


def duplicate_key(row: dict):
    """
    Suppress duplicate map candidates from the same source/date/title.
    Raw rows remain untouched.
    """
    title_n = normalize(row.get("title"))

    if not title_n:
        return None

    return (
        text(row.get("source_id")),
        title_n,
        text(row.get("published_at")),
    )


def main():
    rows, bad_lines = load_rows()

    results = []
    counts = Counter()
    source_counts = Counter()
    seen_candidate_keys = set()

    for row in rows:
        decision, reason = classify(row)

        # Deduplicate only visible/reviewable candidates, never raw storage.
        if decision in {"KEEP", "MAYBE"}:
            key = duplicate_key(row)

            if key is not None:
                if key in seen_candidate_keys:
                    decision = "DROP"
                    reason = "duplicate_title_same_source_date"
                else:
                    seen_candidate_keys.add(key)

        filtered = dict(row)
        filtered["filter_decision"] = decision
        filtered["filter_reason"] = reason
        filtered["filter_version"] = FILTER_VERSION

        results.append(filtered)

        counts[decision] += 1
        source_counts[(row.get("source_id"), decision)] += 1

    OUT.parent.mkdir(parents=True, exist_ok=True)

    with OUT.open("w", encoding="utf-8") as f:
        for row in results:
            f.write(
                json.dumps(row, ensure_ascii=False)
                + "\n"
            )

    print("")
    print("=== TURKEY PULSE FILTER REPORT ===")
    print(f"VERSION: {FILTER_VERSION}")
    print(f"RAW:   {len(rows)}")
    print(f"KEEP:  {counts['KEEP']}")
    print(f"MAYBE: {counts['MAYBE']}")
    print(f"DROP:  {counts['DROP']}")
    print(f"BAD JSON LINES: {bad_lines}")
    print("")
    print("By source:")

    sources = sorted(
        {
            text(row.get("source_id"))
            for row in rows
        }
    )

    for source_id in sources:
        print(
            f"  {source_id}: "
            f"KEEP={source_counts[(source_id, 'KEEP')]} "
            f"MAYBE={source_counts[(source_id, 'MAYBE')]} "
            f"DROP={source_counts[(source_id, 'DROP')]}"
        )

    print("")
    print(f"Wrote: {OUT}")


if __name__ == "__main__":
    main()

...
| ECMWF Open Data (IFS) | GREEN – verified with attribution | N/A for current use | The exact ECMWF Open Data forecast dataset used by the probe/collector is published under CC BY 4.0 together with ECMWF Open Data Terms of Use. ECMWF explicitly permits redistribution and commercial use with appropriate attribution. Our weather layer derives province-level signals from IFS model fields (`2t`, `10u`, `10v`, `tp`). These are **model-derived Turkey Pulse signals**, not official MGM warnings. Evidence: https://www.ecmwf.int/en/forecasts/datasets/open-data and https://confluence.ecmwf.int/pages/viewpage.action?pageId=272330270. |
...
7. ECMWF weather signals must carry ECMWF attribution and be described as model-derived signals, never as official MGM warnings.
8. **RED candidate sources** are not added to the public production pipeline unless the restriction is resolved by explicit permission or licensing.
